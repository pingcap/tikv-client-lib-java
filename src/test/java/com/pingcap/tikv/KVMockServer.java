...
public class KVMockServer extends TikvGrpc.TikvImplBase {
  ...
  private TreeMap<TiKey<ByteString>, ByteString> dataMap = new TreeMap<>();
  ...
  public void put(ByteString key, ByteString value) {
    dataMap.put(TiKey.create(key), value);
  }

  public void remove(ByteString key) {
    dataMap.remove(TiKey.create(key));
  }
  ...
  public void rawGet(
    ...
    try {
      ...
      if (errorCode != null) {
        ...
        builder.setValue(dataMap.get(TiKey.create(key)));
      }
  ...
  public void kvGet(
    ...
    try {
      ...
      if (errorCode != null) {
        ...
        ByteString value = dataMap.get(TiKey.create(key));
  ...
  public void kvScan(
    ...
    try {
      ...
      if (errorCode != null) {
        ...
        SortedMap<TiKey<ByteString>, ByteString> kvs = dataMap.tailMap(TiKey.create(startKey));
        builder.addAllPairs(
            kvs.entrySet()
                .stream()
                .map(
        ...
        if (errorCode != null) {
          ...
          builder.addAllChunks(
              kvs.entrySet()
                  ...
                  .filter(kv -> TiKey.create(kv.getKey()).compareTo(TiKey.create(keyRange.getEnd())) <= 0)
                  .map(
                      kv ->
                          Chunk.newBuilder()
